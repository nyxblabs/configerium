...
import { dirname, extname, resolve } from 'nyxpath'
import createDynot from 'dynot'
import * as rc9 from 'configorium'
import { nyxdefaults } from 'nyxdefaults'
import { findWorkspaceDir, readPackageJSON } from 'pkg-def'
...
export async function loadConfig<
  T extends UserInputConfig = UserInputConfig, MT extends ConfigLayerMeta = ConfigLayerMeta,
>(options: LoadConfigOptions<T, MT>): Promise<ResolvedConfig<T, MT>> {
   ...
   // Create dynot instance
   options.dynot
    = options.dynot
    || createDynot(undefined as unknown as string, {
       interopDefault: true,
       requireCache: false,
       esmResolve: true,
       ...options.dynotOptions,
    })
   ...
   if (options.packageJson) {
      ...
      Object.assign(pkgJson, nyxdefaults({}, ...values))
   }

   // Combine sources
   r.config = nyxdefaults(
      options.overrides,
      config,
      configRC,
      pkgJson,
   ...
   if (options.extend) {
      ...
      r.config = nyxdefaults(r.config, ...r.layers!.map(e => e.config)) as T
   }
   ...
   // Apply defaults
   if (options.defaults)
      r.config = nyxdefaults(r.config, options.defaults) as T
...
>(
   ...
   if (GIT_PREFIXES.some(prefix => source.startsWith(prefix))) {
      const { downloadTemplate } = await import('gitize')
      const url = new URL(source)
      const gitRepo
      = url.protocol + url.pathname.split('/').slice(0, 2).join('/')
      const name = gitRepo.replace(/[#/:@\\]/g, '_')
      const tmpDir = process.env.XDG_CACHE_HOME
         ? resolve(process.env.XDG_CACHE_HOME, 'configerium', name)
         : resolve(homedir(), '.cache/configerium', name)
   ...
   if (NPM_PACKAGE_RE.test(source)) {
      try {
         source = options.dynot!.resolve(source, { paths: [options.cwd!] })
      }
      catch {}
   }
   ...
   try {
      res.configFile = options.dynot!.resolve(resolve(cwd, source), {
         paths: [cwd],
      })
   }
   catch {}
   ...
   res.config = options.dynot!(res.configFile!)
   ...
   if (options.envName) {
      ...
      if (Object.keys(envConfig).length > 0)
         res.config = nyxdefaults(envConfig, res.config)
   }

   // Meta
   res.meta = nyxdefaults(res.sourceOptions!.meta, res.config!.$meta) as MT
   delete res.config!.$meta

   // Overrides
   if (res.sourceOptions!.overrides)
      res.config = nyxdefaults(res.sourceOptions!.overrides, res.config) as T
